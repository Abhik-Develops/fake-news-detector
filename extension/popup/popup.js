...
chrome.storage.local.get(["autoScreen", "includeImages", "includeVideos"], (result) => {
...
function analyzePage(tabId, url, includeImages, includeVideos) {
  ...
  fetch("http://127.0.0.1:8000/analyze", {
    ...
    body: JSON.stringify({ url: url, include_images: includeImages, include_videos: includeVideos }),
  })
...
function displayResult(data) {
  ...
  if (data.final_ai_score >= 75) {
    resultIcon.innerHTML = "✅";
    resultIcon.classList.add("green-tick");
  } else if (data.final_ai_score >= 50) {
    resultIcon.innerHTML = "❓";
    resultIcon.classList.add("orange-question");
  } else {
    resultIcon.innerHTML = "❌";
    resultIcon.classList.add("red-cross");
  }

  const textResult = data.text ? createResultItem("Text Analysis", data.text) : "";
  const imageResult = data.image ? createResultItem("Image Analysis", data.image) : "";
  const videoResult = data.video ? createResultItem("Video Analysis", data.video) : "";
  ...
  resultText.innerHTML = `
        ...
        ${videoResult}
        <div class="result-item">
            <strong>Final Blockchain Score:</strong> ${data.final_blockchain_score}<br>
        </div>
        <div class="result-item">
            <strong>Final AI Score:</strong> ${data.final_ai_score}
        </div>
    `;
...
function createResultItem(title, data) {
  return `
    ...
    <div class="result-item">
        - Phrase Tool: ${getResultIcon(data.phrase_tool)}
    </div>
    <div class="result-item">
        - Language Tool: ${getResultIcon(data.language_tool)}
    </div>
    <div class="result-item">
        - Commonsense Tool: ${getResultIcon(data.commonsense_tool)}
    </div>
    <div class="result-item">
        - Standing Tool: ${getResultIcon(data.standing_tool)}
    </div>
    <div class="result-item">
        - Fact Check Tool: ${getResultIcon(data.fact_check_tool)}
    </div>
  `;
}
...
function getResultIcon(score) {
  if (score >= 75) {
    ...
    return `<span class="result-icon-small orange-question">❓</span>`;
  } else {
